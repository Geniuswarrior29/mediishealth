...
class Program
{
    static void Main()
    {
        ...
        if (!content.Contains("The Power of Ayurveda Section"))
        {
        ...
        // 3. Fix Logo Text and Image globally (accounting for SVG Mockup comment)
        content = Regex.Replace(content, @"(?s)<!-- SVG Logo Mockup -->\s*<svg[^>]+>.*?</svg>\s*Mediis Health", "<img src=\"logo.png\" alt=\"Logo\" style=\"height: 40px; margin-right: 10px; vertical-align: middle;\">\r\n                Mediis Health and Lab");
        
        // If there's any other Mediis Health logo (without SVG Mockup comment), fix it
        content = Regex.Replace(content, @"(?s)(<a href=""[^""]+"" class=""logo"">\s*)(<svg[^>]+>.*?</svg>|<img[^>]+>)\s*(Mediis Health)\b(\s*</a>)", "$1<img src=\"logo.png\" alt=\"Logo\" style=\"height: 40px; margin-right: 10px; vertical-align: middle;\">\r\n                Mediis Health and Lab$4");
        ...
        string bookBtnText = "Book Consultation";
        string bookLink = "book-consultation.html";
        ...
        Console.WriteLine("Index.html fixes applied successfully in C#.");
    }
}
